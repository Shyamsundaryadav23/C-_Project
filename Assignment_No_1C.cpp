...
class student // Defining a class 'student' with private members: name, roll number, and marks
{
    ...
    int mark;     // Marks of the student (or SGPA)
...
public:
    ...
    void read()
    {
        ...
        cout << "Enter the mark: ";
        cin >> mark;                    // Reading the student's marks (SGPA)
    }
    ...
    void display()
    {
        ...
        cout << "Mark: " << mark << endl;   // Display the marks (SGPA)
    }
...
{
    int max = s[0].mark;  // Start by assuming the first student's mark is the maximum
    for (int i = 1; i < n; i++)
    {
        if (s[i].mark > max)  // If current student's mark is greater than 'max', update 'max'
        {
            max = s[i].mark;
        }
    }
...
int main()
{
    ...
    student s[n];  // Array to hold details of 'n' students
